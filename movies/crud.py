...
class Crud(Dbcreate):
    ...
    def select(self, MOVIE_NAME, MOVIE_YEAR):
        fetch = {}
        query = f'''SELECT * FROM MOVIES WHERE MOVIE_NAME LIKE '%{MOVIE_NAME}%' AND MOVIE_YEAR LIKE '%{MOVIE_YEAR}%' '''
        super().cursorr().execute(query)
        fetch = super().cursorr().fetchall()
        ...
        for movie in fetch:
            ...
            print("Movie Poster url: ", movie[3])
            print("Movie description: ", movie[4])
            print("Movie duration: ", hours,"Hours", mints,"mins")
